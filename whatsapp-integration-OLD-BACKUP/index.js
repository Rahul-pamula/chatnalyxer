    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            */
            ...
            // Initial sync happens above, then periodic sync every 5 minutes (300s) to avoid rate limits
            setInterval(async () => {
                console.log('🔄 Auto-syncing groups to catch any new groups...');
                try {
                    await syncGroups();
                } catch (err) {
                    console.log('⚠️ Auto-sync failed (Rate Limit/Network):', err.message);
                }
            }, 300000); // 5 minutes
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            if (msg.message.documentMessage) {
                ...
                mediaType = "pdf"; // Default to pdf for now since user specifically asked for PDF
                // You could check mimetype: doc.mimetype === 'application/pdf'
                msgBody = doc.caption || doc.fileName || "[PDF Document]";
                finalContent = msgBody;

                console.log(`📄 PDF detected: ${doc.fileName}`);
                ...
                try {
                    ...
                    // Save to temp file
                    const fileName = `${Date.now()}_${doc.fileName || 'document.pdf'}`;
                    const absPath = path.resolve(process.cwd(), 'downloads', fileName);
                    ...
                    mediaUrl = absPath;
                    console.log(`✅ Saved PDF to: ${mediaUrl}`);

                    // Extract PDF text via backend
                    try {
                        ...
                        // Continue anyway - PDF is still saved
                    }
            ...
            // 2. Check for Image
                ...
                try {
                    ...
                    await fs.promises.writeFile(absPath, buffer);
                    mediaUrl = absPath;
                    console.log(`✅ Saved Image to: ${mediaUrl}`);
            ...
            try {
                ...
                console.log(`📨 [${groupName}] ${senderName}: ${finalContent.substring(0, 30)}... [Media: ${mediaType}]`);
            } catch (err) {
