...
// Get User ID from args (default to 'admin')
const userId = process.argv[2] || 'admin';
const isRender = process.env.RENDER;

console.log(`🚀 Starting OTP Service for User ID: ${userId}`);

// Auth state folder - dynamic based on user ID
const AUTH_FOLDER = isRender
    ? `/opt/render/.wwebjs-sessions-otp/baileys_auth_info_${userId}`
    : `./auth_info_baileys_${userId}`;
    ...
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (!isClientReady) {
                    ...
                    try {
                        ...
                        await axios.post(`${BASE_URL}/whatsapp/status`, {
                            user_id: process.argv[2] || 1, // Default to admin ID 1 if not specified
                            ready: false,
                ...
                // ✅ Report SUCCESS to backend so Admin Dashboard sees it!
                try {
                    const axios = (await import('axios')).default;
                    const BASE_URL = 'http://localhost:8000';
        ...
        // 📩 MESSAGE LISTENER (Added to enable Admin Bot Analysis)
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            for (const msg of messages) {
                ...
                // Allow Admin Bot to analyze Group Messages
                if (!isGroup) continue;
                ...
                if (!msgBody && !mediaUrl) continue;

                // Send to Backend
                try {
                    const axios = (await import('axios')).default;
                    // Use localhost:8000 directly since we know where backend is
                    const BASE_URL = 'http://localhost:8000';
                    // Use the fixed API Key from backend messages.py
                    ...
                    await axios.post(`${BASE_URL}/messages/from-whatsapp`, {
                        content: msgBody,
                        extracted_content: finalContent !== msgBody ? finalContent : null, // NEW: Send extracted text
...
// Pairing Code Endpoint - Get current code
app.get('/pairing-code', (req, res) => {
    try {
        ...
        res.json({
            ...
            expired: isExpired,
            formatted: isExpired ? null : currentPairingCode.code.match(/.{1,4}/g).join(' ') // Format as "1234 5678"
        });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
// Generate New Pairing Code - Manual trigger
